...
const UserCourses = ({ courses,type }: any) => {
        ...
        <>
          {courses?.map((course: any) => (
            ...
            >
              ...
              {/* courseDetails */}
              <Link
                to={type === "enrolled" ? `/watch-course/${course?._id}/section/${course?.sections?.[0]?._id}/sub-section/${course?.sections?.[0]?.subSections?.[0]}` : `/course/details/${course?._id}`}
